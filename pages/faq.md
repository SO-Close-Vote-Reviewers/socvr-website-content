...
* For `cv-pls` requests: As with most requests, a request reason is required. **It is *highly recommended* that your request reason includes an actual close reason**, even if that's your custom reason (e.g. "Custom: Homework without attempt"). Including an actual close reason significantly increases the likelihood that your request will be acted upon by other users. A request reason of just "off-topic" is insufficient, because it leaves too many possibilities as to why you feel the question should be closed.
